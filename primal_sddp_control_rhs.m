...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%size_f(t): number of components in f(t)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
function [lower_bounds,upper_bounds,time]=primal_sddp_control_rhs(T,size_x,size_u,size_b,size_f,upper_x,lower_x,upper_u,lower_u,costc,costd,bs,fs,probabilities,nb_scenarios_rhs,nb_iter_max,a_subi,a_subj,a_valij,b_subi,b_subj,b_valij,c_subi,c_subj,c_valij,d_subi,d_subj,d_valij,e_subi,e_subj,e_valij,f_subi,f_subj,f_valij,big_m,talpha,tol)
...
dynamic_subi{1,1}=[a_subi{1,1},c_subi{1,1},d_subi{1,1}+size_b(1),f_subi{1,1}+size_b(1),size_b(1)+size_f(1)+1];
dynamic_subj{1,1}=[a_subj{1,1},c_subj{1,1}+size_x(1),d_subj{1,1},f_subj{1,1}+size_x(1),size_x(1)+size_u(1)+1];
dynamic_valij{1,1}=[a_valij{1,1},c_valij{1,1},d_valij{1,1},f_valij{1,1},1];
dynamic_blx{1,1}=[lower_x{1,1};lower_u{1,1};-inf];
dynamic_bux{1,1}=[upper_x{1,1};upper_u{1,1};inf];
dynamic_blc{1,1}=[bs{1,1}{1,1};-inf*ones(size_f(1),1);-big_m];
dynamic_buc{1,1}=[bs{1,1}{1,1};fs{1,1}{1,1};inf];
dynamic_c{1,1}=[costc{1,1};costd{1,1};1];

for t=2:T-1
    dynamic_subi{1,t}=[a_subi{1,t},c_subi{1,t},d_subi{1,t}+size_b(t),f_subi{1,t}+size_b(t),size_b(t)+size_f(t)+1];
    dynamic_subj{1,t}=[a_subj{1,t},c_subj{1,t}+size_x(t),d_subj{1,t},f_subj{1,t}+size_x(t),size_x(t)+size_u(t)+1];
    dynamic_valij{1,t}=[a_valij{1,t},c_valij{1,t},d_valij{1,t},f_valij{1,t},1];
    dynamic_blx{1,t}=[lower_x{1,t};lower_u{1,t};-inf];
    dynamic_bux{1,t}=[upper_x{1,t};upper_u{1,t};inf];
    dynamic_blc{1,t}=[zeros(size_b(t),1);-inf*ones(size_f(t),1);-big_m];
    dynamic_buc{1,t}=[zeros(size_b(t),1);zeros(size_f(t),1);inf];
    dynamic_c{1,t}=[costc{1,t};costd{1,t};1];
end

dynamic_subi{1,T}=[a_subi{1,T},c_subi{1,T},d_subi{1,T}+size_b(T),f_subi{1,T}+size_b(T)];
dynamic_subj{1,T}=[a_subj{1,T},c_subj{1,T}+size_x(T),d_subj{1,T},f_subj{1,T}+size_x(T)];
dynamic_valij{1,T}=[a_valij{1,T},c_valij{1,T},d_valij{1,T},f_valij{1,T}];
dynamic_blx{1,T}=[lower_x{1,T};lower_u{1,T}];
dynamic_bux{1,T}=[upper_x{1,T};upper_u{1,T}];
dynamic_blc{1,T}=[zeros(size_b(T),1);-inf*ones(size_f(T),1)];
dynamic_buc{1,T}=[zeros(size_b(T),1);zeros(size_f(T),1)];
...
while End_Algo
    ...
    length(dynamic_subi{1,1})
    length(dynamic_subj{1,1})
    length(dynamic_valij{1,1})
    max(dynamic_subi{1,1})
    max(dynamic_subj{1,1})
    size_b(1)+size_f(1)+iter
    size_x(1)+size_u(1)+1
    prob.a=sparse(dynamic_subi{1,1},dynamic_subj{1,1},dynamic_valij{1,1},size_b(1)+size_f(1)+iter,size_x(1)+size_u(1)+1);
    ...
    for t=2:T-1
        ...
        aux2=fs{1,t}{1,Index}-sparse(e_subi{1,t-1},e_subj{1,t-1},e_valij{1,t-1},size_f(t),size_x(t-1))*trial_states{1,t-1};
        dynamic_buc{1,t}(1:size_b(t)+size_f(t))=[aux1;aux2];
        dynamic_blc{1,t}(1:size_b(t)+size_f(t))=[aux1;-inf*ones(size_f(t),1)];
        prob.buc=dynamic_buc{1,t};
        prob.blc=dynamic_blc{1,t};
        prob.a=sparse(dynamic_subi{1,t},dynamic_subj{1,t},dynamic_valij{1,t},size_b(t)+size_f(t)+iter,size_x(t)+size_u(t)+1);
        [~,res]=mosekopt('minimize echo(0)',prob);
        sol=res.sol.bas.xx;
        solsta=strcat('MSK_SOL_STA_', res.sol.bas.solsta);
        if (strcmp(solsta,'MSK_SOL_STA_PRIMAL_INFEASIBLE_CER')==1)
    ...
    prob.buc=dynamic_buc{1,T};
    prob.blc=dynamic_blc{1,T};
    prob.a=sparse(dynamic_subi{1,T},dynamic_subj{1,T},dynamic_valij{1,T},size_b(T)+size_f(T),size_x(T)+size_u(T));
    [~,res]=mosekopt('minimize echo(0)',prob);
    sol=res.sol.bas.xx;
    solsta=strcat('MSK_SOL_STA_', res.sol.bas.solsta);
    if (strcmp(solsta,'MSK_SOL_STA_PRIMAL_INFEASIBLE_CER')==1)
    ...
    for t=T:-1:2
        if (t==T)
            ...
            for j=1:nb_scenarios_rhs(T)
                ...
                aux2=fs{1,T}{1,j}-(sparse(e_subi{1,T-1},e_subj{1,T-1},e_valij{1,T-1},size_f(T),size_x(T-1)))*trial_states{1,T-1};
                prob.buc=[aux1;aux2];
                prob.blc=[aux1;-inf*ones(size_f(T),1)];
                prob.c=dynamic_c{1,T};
                prob.blx=dynamic_blx{1,T};
                prob.bux=dynamic_bux{1,T};
                prob.a = sparse(dynamic_subi{1,T},dynamic_subj{1,T},dynamic_valij{1,T},size_b(T)+size_f(T),size_x(T)+size_u(T));
                ...
                elseif (strcmp(solsta,'MSK_SOL_STA_DUAL_INFEASIBLE_CER')==1)
                    ...
                    auxs=-sparse(b_subj{1,T-1},b_subi{1,T-1},b_valij{1,T-1},size_x(T-1),size_b(T))*(dual1(1:size_b(T))-dual2(1:size_b(T)))+sparse(e_subj{1,T-1},e_subi{1,T-1},e_valij{1,T-1},size_x(T-1),size_f(T))*dual2(size_b(T)+1:size_b(T)+size_f(T));
                    aux=(dual1(1:size_b(T))-dual2(1:size_b(T)))'*prob.buc(1:size_b(T))-dual2(size_b(T)+1:size_b(T)+size_f(T))'*prob.buc(size_b(T)+1:size_b(T)+size_f(T))-auxs'*trial_states{1,T-1};
            ...
            for j=1:nb_scenarios_rhs(t)
                ...
                prob.buc=dynamic_buc{1,t};
                prob.blc=dynamic_blc{1,t};
                prob.a=sparse(dynamic_subi{1,t},dynamic_subj{1,t},dynamic_valij{1,t},size_b(t)+size_f(t)+iter+1,size_x(t)+size_u(t)+1);
                [~,res]=mosekopt('minimize echo(0)',prob);
                sol=res.sol.bas.xx;
                solsta=strcat('MSK_SOL_STA_', res.sol.bas.solsta);
                if (strcmp(solsta,'MSK_SOL_STA_PRIMAL_INFEASIBLE_CER')==1)
                ...
                elseif (strcmp(solsta,'MSK_SOL_STA_DUAL_INFEASIBLE_CER')==1)
                    ...
                    dual4=res.sol.bas.sux;
                    auxs=-sparse(b_subj{1,t-1},b_subi{1,t-1},b_valij{1,t-1},size_x(t-1),size_b(t))*(dual1(1:size_b(t))-dual2(1:size_b(t)))+sparse(e_subj{1,t-1},e_subi{1,t-1},e_valij{1,t-1},size_x(t-1),size_f(t))*dual2(size_b(t)+1:size_b(t)+size_f(t));
                    aux=(dual1(1:size_b(t))-dual2(1:size_b(t)))'*prob.buc(1:size_b(t))-dual2(size_b(t)+1:size_b(t)+size_f(t))'*prob.buc(size_b(t)+1:size_b(t)+size_f(t))+dual1(size_b(t)+size_f(t)+1:size_b(t)+size_f(t)+iter+1)'*prob.blc(size_b(t)+size_f(t)+1:size_b(t)+size_f(t)+iter+1)-auxs'*trial_states{1,t-1};
                    for i=1:(size_x(t)+size_u(t))
                        if (prob.blx(i)~=-inf)
                            aux=aux+prob.blx(i)*dual3(i);
                        end
        ...
        dynamic_subi{1,t-1}=[dynamic_subi{1,t-1},(size_b(t-1)+size_f(t-1)+iter+1)*ones(1,size_x(t-1)+1)];
